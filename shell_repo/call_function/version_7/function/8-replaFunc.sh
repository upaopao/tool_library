...
set FILE_PATH [ exec cat  $DB_FILE_FULL/$DATA_DB | tail -n +4 | head -n 1 ]
set IP_COUNT  [ exec cat  $DB_FILE_FULL/$DATA_DB | tail -n +5 | head -n 1 ]
#set IP_ADDR_A [ exec cat  $DB_FILE_FULL/$DATA_DB | tail -n +6 | head -n 1 ]

# in address in the file $DB_FILE start line is 6
set ip_line 6
