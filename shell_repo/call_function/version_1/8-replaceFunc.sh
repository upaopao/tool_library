...
# src & dst file path
set binary_file_src_path     /root/single-file
...
set EXEC_FILE_ADDR [ exec cat ./$DB_FILE | tail -n +1 | head -n 1 ]
set FILE_NAME [ exec cat ./$DB_FILE | tail -n +2 | head -n 1 ]
set FILE_TYPE [ exec cat ./$DB_FILE | tail -n +3 | head -n 1 ]
set IP_COUNT  [ exec cat ./$DB_FILE | tail -n +4 | head -n 1 ]
#set IP_ADDR_A [ exec cat ./$DB_FILE | tail -n +5 | head -n 1 ]
