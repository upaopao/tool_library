...
### touch the catalog and the MarkDown File's update-times

# get main catalog_name
cd $ENV_PATH >> /dev/null;
cd ../ >> /dev/null;
main_catalog_name=`ls -alF | grep '/' | awk {print'$9'}`;

# touch the main catalog_name
for mcata_name in ${main_catalog_name[@]}
do
	# './' and '../' exclude, operate the next ring-cycle.
	if [ $mcata_name == "./" ];then
		continue;
	fi
	...
	# update the catalog times
	touch -d "$new_time" $mcata_name
done

# touch/update the MarkDown File's times
touch -d "$new_time" *.md

# change catalog to the '../',get sub catalog_name,restore the file exec_path(ENV_PATH)
cd - >> /dev/null;
sub_catalog_name=`ls -alF | grep '/' | awk {print'$9'}`;

# touch the sub catalog_name
for scata_name in ${sub_catalog_name[@]}
do
	# './' and '../' exclude, operate the next ring-cycle.
	if [ $scata_name == "./" ];then
		continue;
